...
def get_hidden_word_with_visible_guessed_letters(word, guessed_letters):
    visible_guessed = list()
    for letter in word:
      if check_letter_in_word(letter,"".join(guessed_letters) ):
          visible_guessed.append(letter)
      else:
          visible_guessed.append("_")
    return " ".join(visible_guessed)
...
def alphabet_display_with_guessed_letters_marked(letters_alphabet, guessed_letters):
    letters_with_space = list()
    for letter in letters_alphabet:
        if letter in guessed_letters:
            letters_with_space.append(letter + "\u0336")
        else:
            letters_with_space.append(letter)
    return  " ".join(letters_with_space)
